...
bool ProcessExists(std::string ProcessName) {
	HackIH MemInstance;
	for (auto& proc : MemInstance.GetProcesses()) {
		if (std::get<1>(proc) == ProcessName) {
			return true;
		}
	}
	return false;
...
void SetValue(std::string processname, std::string adress, std::string value) {
	HackIH MemInstance;
	MemInstance.SetDebugOutput(std::cout);
	MemInstance.bind(processname);
	PointerIH UserPointer = { MemInstance.BaseAddress };

	std::vector<std::string> Offsets;
	std::string delimiter = "+";
	if (adress.find(delimiter))
	{
		size_t pos = 0;
		std::string token;
		while ((pos = adress.find(delimiter)) != std::string::npos) {
			token = adress.substr(0, pos);
			Offsets.push_back(token);
			adress.erase(0, pos + delimiter.length());
		}
		Offsets.push_back(adress);
		std::string c;
		for (int i = 0; i < Offsets.size(); i++) {
			//if (!c.find("0x") != std::string::npos) {
				//Offsets[i] = "0x" + Offsets[i];
			//}
			UserPointer << std::stoi(Offsets[i], 0, 16);
		}
	}
	else
	{
		UserPointer << std::stoi(adress, 0, 16);
	}
	if (std::string(value).compare("0") == 0) {
		MemInstance.Write(UserPointer, false);
	}
	else if (std::string(value).compare("1") == 0) {
		MemInstance.Write(UserPointer, true);
	}
	else {
		...
		if (!isNumber(value)) {
			MemInstance.Write(UserPointer, value);
		}
		else if (value.find(".") != std::string::npos) {
			MemInstance.Write(UserPointer, std::stof(value));
		}
		else {
			MemInstance.Write(UserPointer, std::stoi(value));
		}
	}
}


std::string ReadValue(std::string processname, std::string adress, std::string type) {
	HackIH MemInstance;
	MemInstance.bind(processname);
	PointerIH UserPointer = { MemInstance.BaseAddress };
	std::vector<std::string> Offsets;
	...
	if (std::string(type).compare("float") == 0) {
		return std::to_string(MemInstance.Read<float>(UserPointer));
	} 
	else if (std::string(type).compare("byte") == 0) {
		return std::to_string(MemInstance.Read<bool>(UserPointer));
	} 
	else if (std::string(type).compare("string") == 0) {
		return MemInstance.Read<std::string>(UserPointer);
	}
	else {
		return std::to_string(MemInstance.Read<int>(UserPointer));
	}
}